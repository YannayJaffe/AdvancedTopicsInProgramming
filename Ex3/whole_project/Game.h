...
{
public:
    Game(std::unique_ptr<PlayerAlgorithm> player1,
         std::unique_ptr<PlayerAlgorithm> player2); // provide the inputs from the command line in order to determine the game mode

    int runGame(); // returns the winner id. 0 for tie
...
private:
    ...
    int
    gameInit(); //this method performs all initialization and returns winner id or 0 if tie. -1 if no winner after init
    ...
    void initPlayerOnBoard(
            int playerId); // this method initializes only the player whos id is playerId on the board (and conducts fights...)
    ...
    getPlayerPiece(int playerId, const Point &point); // returns the wanted player piece, nullptr if not found
    ...
    playMove(const Move &move,
             int playerId); //plays the move and updates player's piece vectors accordingly. returns the fight info
