...
{
    thread_local std::unique_ptr<PlayerAlgorithm> player1;
    thread_local std::unique_ptr<PlayerAlgorithm> player2;
    thread_local bool updateScoreForPlayer2;
    thread_local std::string player1Id;
    thread_local std::string player2Id;
    ...
    while (true)
    {
        ...
        thread_local Game game(std::move(player1),std::move(player2));
        thread_local int result = game.runGame();

        {
            std::lock_guard<std::mutex> lg(scoreBoardLock); // acquire lock
            updateScore(player1Id,player2Id,result,updateScoreForPlayer2);
        }
    }
                                      ...
                                      bool &updateScoreForPlayer2)
{
    ...
    } else
    {
        ...
        for (int i = 0; i < static_cast<int>(unfinishedPlayers.size()); i++)
        {
            if (i != player1Idx)
            {
...
void TournamentManager::updateScore(const std::string &player1Id, const std::string &player2Id, int result,
                                    bool updateScoreForPlayer2)
{
    switch (result)
    {
        case 1:
            scoreBoard[player1Id].score += pointsForWin;
            break;
        case 2:
            if (updateScoreForPlayer2)
                scoreBoard[player2Id].score += pointsForWin;
            break;
        case 0:
            scoreBoard[player1Id].score += pointsForTie;
            if (updateScoreForPlayer2)
                scoreBoard[player2Id].score += pointsForTie;
            break;
        default:
        {}
    }
