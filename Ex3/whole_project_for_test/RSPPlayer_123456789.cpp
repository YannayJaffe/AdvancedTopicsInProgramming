...
void RSPPlayer_123456789::getInitialPositions(int player, std::vector<unique_ptr<PiecePosition>>& vectorToFill)
{
    myId = player;
...
void RSPPlayer_123456789::notifyOnInitialBoard(const Board& b, const std::vector<unique_ptr<FightInfo>>& fights)
{
    board = b;
...
void RSPPlayer_123456789::notifyOnOpponentMove(const Move& move)
{
...
void RSPPlayer_123456789::notifyFightResult(const FightInfo& fightInfo)
{
...
unique_ptr<JokerChange> RSPPlayer_123456789::getJokerChange()
{
...
RSPPlayer_123456789::RSPPlayer_123456789() : board(totalXVals, totalYVals)
{

}

std::unique_ptr<PiecePosition> RSPPlayer_123456789::generatePiece(const Board& board, const PieceCounter& counter)
{
...
PointImpl RSPPlayer_123456789::findEmptySpot(const Board& board)
{
...
std::unique_ptr<PiecePosition>& RSPPlayer_123456789::getPiece(std::vector<std::unique_ptr<PiecePosition>>& pieces, const Point& location)
{
...
bool RSPPlayer_123456789::setNoFlag(const Point& point)
{
...
bool RSPPlayer_123456789::updatePosition(std::vector<std::unique_ptr<PiecePosition>>& pieces, const Move& move)
{
...
bool RSPPlayer_123456789::updatePieceType(std::vector<std::unique_ptr<PiecePosition>>& pieces, const Point& point, char type, char jokerType)
{
...
void RSPPlayer_123456789::removePiece(std::vector<std::unique_ptr<PiecePosition>>& pieces, const Point& point)
{
    auto& piece = getPiece(pieces, point);
    piece = nullptr;
}

std::unique_ptr<Point> RSPPlayer_123456789::chooseFrom() const
{
...
std::unique_ptr<Point> RSPPlayer_123456789::chooseTo(const Point& from) const
{
    if (!inBoard(from))
        return std::make_unique<PointImpl>(-1, -1);
...
std::vector<BoardSpot> RSPPlayer_123456789::getAvailableMoves(const Point& from) const
{
...
bool RSPPlayer_123456789::inBoard(const Point& p) const
{
    return (p.getX() >= 1 && p.getX() <= totalXVals && p.getY() >= 1 && p.getY() <= totalYVals);
}

bool RSPPlayer_123456789::isMovable(const PiecePosition& piece) const
{
    return isMovable(piece.getPiece(), piece.getJokerRep());
}

char RSPPlayer_123456789::getStrongerType(char type) const
{
    switch (type)
    {
        case 'R':
...
bool RSPPlayer_123456789::isMovable(char type, char jokerRep) const
{
...
RSPPlayer_123456789::getPiece(const std::vector<std::unique_ptr<PiecePosition>>& pieces, const Point& location) const
{
...
int RSPPlayer_123456789::getRemaining(const std::vector<std::unique_ptr<PiecePosition>>& pieces) const
{
    int cnt = 0;
    for (const auto& piece:pieces)
    {
...
char RSPPlayer_123456789::getEffectiveType(char type, char jokerRep) const
{
    if (type == 'J')
        return jokerRep;
    return type;
...
std::unique_ptr<Point> RSPPlayer_123456789::testAssumedFlagNeighbour(const std::vector<BoardSpot>& neighbours) const
{
    for (const auto& neighbour:neighbours)
    {
...
std::unique_ptr<Point> RSPPlayer_123456789::testOnlyFlagsRemaining(const std::vector<BoardSpot>& neighbours) const
{
...
std::unique_ptr<Point> RSPPlayer_123456789::testWeakerNeighbour(const std::vector<BoardSpot>& neighbours, char myPieceType) const
{
    for (const auto& neighbour:neighbours)
    {
        if (neighbour.getOwner() != opponentId)
...
std::unique_ptr<Point> RSPPlayer_123456789::testUnknownNonFlagNeighbour(const std::vector<BoardSpot>& neighbours) const
{
    for (const auto& neighbour:neighbours)
    {
...
std::unique_ptr<Point> RSPPlayer_123456789::testFreeSpot(const std::vector<BoardSpot>& neighbours) const
{
    for (const auto& neighbour:neighbours)
    {
